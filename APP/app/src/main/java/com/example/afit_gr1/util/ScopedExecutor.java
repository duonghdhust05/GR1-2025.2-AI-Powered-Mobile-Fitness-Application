 ...
 * 
 * **Meaning & Role**: Acts as a lifecycle-aware thread manager for the ML Kit detection tasks.
 * 
 * **Workflow**: 
 * 1. Wraps the Android Main Thread Executor.
 * 2. When ML Kit tasks succeed or fail, they are posted back via this executor.
 * 3. If the user leaves the fragment, `shutdown()` is called. Any pending ML Kit callbacks 
 *    are intercepted and discarded, preventing memory leaks or `NullPointerException`s on dead views.
 * 
 * **Coupling**:
 * - Coupled to Java's `Executor` and `AtomicBoolean`.
 * - Used exclusively by `VisionProcessorBase`.
 * 
 * **Cohesion**: 
 * - High cohesion. Purely responsible for concurrent thread cancellation.
 */
